...
const chatState = {
    ...
    filteredChats: [],  // Чаты после фильтрации/поиска
    displayedCount: 10,  // Количество отображаемых чатов
    chatsPerPage: 10,    // Чатов на одну "страницу"
    isTyping: false,
    abortController: null,  // Контроллер для отмены запросов
    templates: [],
    displayedTemplatesCount: 10,  // Количество отображаемых шаблонов
    templatesPerPage: 10,         // Шаблонов на одну "страницу"
    editingTemplateId: null,
    allModels: [],
    favoriteModels: [],
    currentModel: '',  // Загружается из localStorage при инициализации
    agentCategory: '',  // '' = обычный чат, или ID категории (critical_alerts, trend_analysis, и т.д.)
    availableCategories: {}  // Доступные категории агентов
};
...
async function switchToChat(chatId) {
    try {
        ...
        // Очищаем состояние сообщений перед загрузкой новых
        chatState.messages = [];
        ...
        if (chatMessages) {
            ...
            if (!chatData.messages || chatData.messages.length === 0) {
                ...
                // addMessage сам добавит в chatState.messages в правильном формате
...
// Показать приветственное сообщение для нового чата
function showWelcomeMessage() {
    const chatMessages = document.getElementById('chatMessages');
    if (!chatMessages) return;

    // Определяем название агента
    let agentName = 'Обычный чат';
    let agentDescription = 'Общение с AI без использования модулей аналитики';
    let agentIcon = '💬';
    ...
    const welcomeHTML = `
        <div class="welcome-message">
            ...
            <h2>Выбран режим: ${agentName}</h2>
            <p>${agentDescription}</p>
            <p style="margin-top: 1rem; color: var(--text-muted);">Задайте свой вопрос или опишите задачу...</p>
        </div>
    `;
...
async function deleteAllChats() {
    ...
    try {
        ...
        alert('Ошибка при удалении всех чатов');
    }
}
...
async function initChat() {
    // Сворачивание/разворачивание сайдбара в компактный режим
    const sidebarToggle = document.getElementById('sidebarToggle');
    const sidebarToggleMobile = document.getElementById('sidebarToggleMobile');
    const chatSidebar = document.getElementById('chatSidebar');

    // Загрузка сохраненного состояния сайдбара из localStorage
    ...
    // Основная кнопка toggle - переключает компактный режим
    if (sidebarToggle) {
    ...
    // Клик по заголовкам секций (templates, history) раскрывает сайдбар
    const sectionHeaders = document.querySelectorAll('.section-header[data-section]');
    sectionHeaders.forEach(header => {
        header.addEventListener('click', (e) => {
            // Если сайдбар в компактном режиме - раскрываем его
            if (chatSidebar.classList.contains('compact')) {
                // Проверяем что клик не по кнопке добавления шаблона
    ...
    // Инициализация выбора агента
    initAgentSelector();

    // Загрузка списка чатов из БД
    await loadChats();

    // Инициализация состояния контролов
    updateControlsState();
}
...
async function sendMessage() {
    ...
    // Если нажали во время генерации - останавливаем
    ...
    try {
        ...
        const selectedModel = modelSelect ? modelSelect.value : (chatState.currentModel || 'openai/gpt-4.1-mini');
        const contextMessages = contextLimit ? parseInt(contextLimit.value) : 10;
        const useModulesMode = chatState.agentCategory !== '';  // Если категория выбрана - используем модули

        // Логирование для отладки
        console.log('Sending message with model:', selectedModel);
        console.log('modelSelect.value:', modelSelect ? modelSelect.value : 'N/A');
        console.log('chatState.currentModel:', chatState.currentModel);
        ...
        // Ограничиваем историю согласно настройке контекста
        const chatHistory = allHistory.slice(-contextMessages);

        // Логируем настройки контекста
        console.log('contextLimit:', contextMessages, '| history length:', chatHistory.length);
        ...
        // Если запрос был отменен - не показываем ошибку
...
function addMessage(sender, text, saveToDB = true) {
    ...
    // Сохраняем оригинальный markdown для копирования
    ...
    // Обновляем состояние контролов (блокируем если есть сообщения)
    updateControlsState();
}
...
// Изменить кнопку отправки на Stop
...
// Вернуть кнопку отправки в обычное состояние
...
function stopGeneration() {
    ...
    hideTypingIndicator();
    setSendButtonToNormal();
    addMessage('ai', '⚠️ Генерация остановлена пользователем.');
}
...
function updateTokenInfo(usage) {
    ...
    if (tokenInfo && tokenCount && usage) {
        // Если токены 0 (режим агента) - скрываем badge
...
function copyMessage(button) {
    ...
    // Копируем оригинальный markdown, если доступен, иначе текст
...
// Рендеринг списка шаблонов с пагинацией
function renderTemplates() {
    ...
    // API уже возвращает шаблоны отсортированными от новых к старым
    // Но на всякий случай сортируем ещё раз
    const sortedTemplates = [...chatState.templates].sort((a, b) => {
        return new Date(b.created_at) - new Date(a.created_at);
    });

    // Отображаем только первые N шаблонов
...
async function saveTemplate() {
    ...
    try {
        ...
        if (chatState.editingTemplateId) {
            ...
            chatState.templates.unshift(newTemplate); // Добавляем в начало
        }
...
async function initModelSelector() {
    ...
    if (modelSelect) {
        modelSelect.addEventListener('change', (e) => {
            ...
            console.log('Saved to localStorage:', localStorage.getItem('selectedModel'));
            updateModelPrice();
        });
    }
    ...
    if (contextLimit) {
        ...
        const savedContext = localStorage.getItem('contextLimit');
        if (savedContext) {
            contextLimit.value = savedContext;
            console.log('Context limit loaded from localStorage:', savedContext);
        }

        // Сохраняем при изменении (input - сразу при вводе, change - при потере фокуса)
        const saveContext = (e) => {
            localStorage.setItem('contextLimit', e.target.value);
            console.log('Context limit saved:', e.target.value);
        };

        contextLimit.addEventListener('input', saveContext);
        contextLimit.addEventListener('change', saveContext);
    }

    // Загружаем модели в фоне для отображения цены
    try {
        ...
        if (response.ok) {
            ...
            // Обновляем селект и цену после загрузки моделей
            renderModelSelect();
            updateModelPrice();
        }
    } catch (error) {
        console.error('Ошибка загрузки моделей при инициализации:', error);
        // Всё равно пытаемся обновить цену
        updateModelPrice();
    }
}
...
function loadFavoriteModels() {
    ...
    // Восстанавливаем выбранную модель (любую сохраненную, не только из избранных)
    if (selectedModel) {
        chatState.currentModel = selectedModel;
    } else if (chatState.favoriteModels.length > 0) {
        // Если нет сохраненной, используем первую из избранных
        chatState.currentModel = chatState.favoriteModels[0].id;
    }
}
...
function renderModelSelect() {
    ...
    // Если текущая модель не в избранных, добавляем её
    if (chatState.currentModel && !addedModels.has(chatState.currentModel)) {
        // Ищем модель в allModels
    ...
    // Если нет избранных и нет текущей модели, добавляем дефолтную
    if (chatState.favoriteModels.length === 0 && !chatState.currentModel) {
        const option = document.createElement('option');
        option.value = 'openai/gpt-4.1-mini';
        option.textContent = 'GPT-4.1 Mini (default)';
        option.selected = true;
        modelSelect.appendChild(option);
    }
}
...
async function openModelModal() {
    // Блокируем если в чате уже есть сообщения
    if (chatState.messages.length > 0) {
        alert('Нельзя менять модель в середине диалога. Создайте новый чат для смены модели.');
        return;
    }
    ...
    if (chatState.allModels.length === 0) {
        ...
        // Модели уже загружены - скрываем loading и показываем список
...
function createModelItem(model) {
    ...
    // Обработчик клика на модель для выбора
    ...
    favoriteBtn.addEventListener('click', (e) => {
        e.stopPropagation(); // Чтобы не срабатывал клик на модель
...
// ========== Функции для работы с выбором агента ==========

// Инициализация выбора агента
async function initAgentSelector() {
    // Загружаем сохраненную категорию из localStorage
    loadAgentCategory();

    // Загружаем категории с сервера для корректного отображения названий
    try {
        ...
        console.error('Ошибка загрузки категорий агентов:', error);
    }

    // Обновляем label кнопки (после загрузки категорий!)
    updateAgentLabel();
    ...
    // === Инициализация модального окна промптов ===
    ...
    // Закрытие модального окна промптов по клику на overlay
...
function loadAgentCategory() {
    ...
    if (storedCategory !== null) {
        ...
        saveAgentCategory(); // Сохраняем в localStorage
    }
}
...
// Обновление состояния контролов (блокировка если есть сообщения)
function updateControlsState() {
    ...
    if (selectAgentBtn) {
        if (hasMessages) {
            ...
            selectAgentBtn.title = 'Нельзя менять агента в середине диалога';
        } else {
            selectAgentBtn.disabled = false;
            selectAgentBtn.style.opacity = '1';
            selectAgentBtn.style.cursor = 'pointer';
            selectAgentBtn.title = 'Выбрать специализированного агента';
        }
    }
    ...
    if (selectModelBtn) {
        if (hasMessages) {
            ...
            selectModelBtn.title = 'Нельзя менять модель в середине диалога';
        } else {
            selectModelBtn.disabled = false;
            selectModelBtn.style.opacity = '1';
            selectModelBtn.style.cursor = 'pointer';
            selectModelBtn.title = 'Выбрать модель';
        }
    }
    ...
    if (contextLimit) {
        ...
        if (hasMessages) {
            ...
            contextLimit.title = 'Нельзя менять контекст в середине диалога';
        } else {
            contextLimit.style.opacity = '1';
            contextLimit.style.cursor = 'text';
            contextLimit.title = 'Количество сообщений истории для передачи в AI';
        }
    }
}
...
async function openAgentModal() {
    // Блокируем если в чате уже есть сообщения
    if (chatState.messages.length > 0) {
        alert('Нельзя менять агента в середине диалога. Создайте новый чат для смены агента.');
        return;
    }
    ...
    // Если категории еще не загружены - загружаем
...
// Рендеринг списка категорий агентов
function renderAgentCategories() {
    ...
    // "universal" всегда первый, остальные по алфавиту
    ...
    sortedCategories.forEach(([categoryId, categoryInfo]) => {
        ...
        // Для "universal" не показываем кнопку "Подробнее"
        const showDetailsBtn = categoryId !== 'universal';

        categoryDiv.innerHTML = `
            <div class="agent-category-header">
                <div class="agent-category-icon">${categoryInfo.icon || '🤖'}</div>
                <div class="agent-category-info">
                    <h4>${categoryInfo.name}</h4>
                    <p>${categoryInfo.description}</p>
                    <span class="agent-category-modules">${categoryInfo.modules_count} ${categoryInfo.modules_count === 1 ? 'модуль' : categoryInfo.modules_count < 5 ? 'модуля' : 'модулей'}</span>
                </div>
            </div>
            <div class="agent-category-actions">
                ...
                <button class="btn-agent-prompts" onclick="showAgentPrompts('${categoryId}'); event.stopPropagation();" title="Просмотр и редактирование системных промптов">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                        <path d="M14 2v6h6M16 13H8M16 17H8M10 9H8"/>
                    </svg>
...
function selectAgentCategory(categoryId) {
    ...
    // Если в чате нет сообщений - обновляем приветственное сообщение
    if (chatState.messages.length === 0) {
        showWelcomeMessage();
    }
...
// Показать детали категории агента
async function showCategoryDetails(categoryId) {
    try {
        // Получаем детальную информацию о модулях категории
...
function showDetailsModal(categoryInfo, modules) {
    ...
    modal.innerHTML = `
        ...
        <div class="details-modal-body">
            ...
            <div class="modules-count-badge">${categoryInfo.modules_count} ${categoryInfo.modules_count === 1 ? 'модуль' : categoryInfo.modules_count < 5 ? 'модуля' : 'модулей'}</div>
...
// Показать модальное окно промптов агента
async function showAgentPrompts(categoryId) {
    try {
        ...
        // Обновляем заголовок
        document.getElementById('promptModalTitle').textContent = `Промпты: ${categoryName}`;
    ...
    if (currentPromptData.is_custom) {
        statusDiv.innerHTML = `
            <div class="prompt-status-custom">
                ...
                <span>Используется кастомный промпт. Вы можете вернуться к дефолтному, нажав "Сбросить к дефолтному".</span>
            </div>
        `;
    } else {
        statusDiv.innerHTML = `
            <div class="prompt-status-default">
                ...
                <span>Используется дефолтный промпт. Вы можете отредактировать и сохранить свою версию.</span>
            </div>
        `;
    }
}

// Закрыть модальное окно промптов
function closePromptModal() {
    const modal = document.getElementById('promptModal');
    if (modal) {
        modal.classList.remove('active');
    }
    // Сбрасываем активную вкладку на "Текущий"
...
async function resetPrompt() {
    if (!confirm('Вы уверены, что хотите сбросить промпт к дефолтному значению? Ваши изменения будут потеряны.')) {
        return;
    }
    ...
    try {
        ...
        alert('Промпт успешно сброшен к дефолтному');

    } catch (error) {
        console.error('Error resetting prompt:', error);
        alert('Ошибка при сбросе промпта');
    }
}
